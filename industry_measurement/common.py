    ...
    def run(self):
        ...
        while self._running:
            ...
            if not self.cfg["cliped"]:
                flag, cliped_img, _ = extract(cliped_img, self, self.cfg["round"])
                if(flag == False):
                    print("提取桌面失败， 重新启动")
                    self.cnt += 1
                    continue
            ...
            # 第二步，检测并计算零件尺寸
                # 螺钉
            bbox_screw, keypoints_screw = self.screw_poser.run(cliped_img)
            results_screw = calculate_screw(cliped_img,  bbox_screw, keypoints_screw, depth_img=cliped_depth_img, round=self.cfg["round"])

                # 垫片
            bbox_shim, masked_img, mask = self.shim_segor.save_mask(cliped_img)
            
            results_list = fit_and_draw_circles(mask, bbox_shim, round=self.cfg["round"])  # result 格式为：[(inner_center, inner_radius, outer_center, outer_radius), ...]
            results_shim = calculate_shim(cliped_img, results_list, round=self.cfg["round"])

            # 第三步，合并结果，发送，保存
            results = merge_results(results_screw, results_shim)
            ...
            if self.cfg["judge"]:
                ...
                self.tcp_client.send(bytes.fromhex("00000002" + ans_len + hexstr))
                self.tcp_client.close()

            # 保存结果
            file_name = self.cfg["save_path"]+ f"/XJTU-xjtuwhfz-R{self.cfg['round']}.txt"
            with open(file_name, "w", encoding="utf-8", ) as file:
                file.write(results)
                msg = "结果已保存至: " + file_name
                self.status_sign.emit(msg)
                                      
            # 第四步，可视化
            self.bar_msg.emit("finished")
            visualized_img = visualize_bbox(cliped_img.copy(), bbox_screw, bbox_shim)

            # 比赛的时候只展示框，但是保存详细的可视化以供DEBUG
            from visualize import visualize_with_keypoints, visualize_with_seg
            v1 = visualize_with_keypoints(cliped_img.copy(), bbox_screw, keypoints_screw)
            v2 = visualize_with_seg(v1, bbox_shim, mask, None)
            cv2.imwrite("./saves/visualized.png", v2)
            
            self.img_sign.emit(visualized_img)
